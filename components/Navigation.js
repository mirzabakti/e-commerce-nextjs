...
const Navigation = () => {
  ...
  let { user, setUser, fetchCheckoutStatus, setFetchCheckoutStatus, getCheckoutUser, setCheckoutUser } = state;
            ...
            <ul className="mt-4 flex flex-col rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800 md:mt-0 md:flex-row md:space-x-8 md:border-0 md:bg-white md:text-sm md:font-medium md:dark:bg-gray-900">
              <li className="border-r-2 border-gray-700 pr-5">
                <Link href="/user/checkout">
                  <a className="relative block rounded bg-blue-700 py-2 pr-4 pl-3 text-white dark:text-white md:bg-transparent md:p-0 md:text-blue-700" aria-current="page">
                    <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path
                        strokeLinecap="round"
                    ...
                    <div className="absolute -top-2 -right-2 inline-flex h-5 w-5 items-center justify-center rounded-full border-2 border-white bg-red-500 text-xs font-bold text-white dark:border-gray-900">{getCheckoutUser}</div>
                  </a>
                </Link>
              </li>
              {!user && (
